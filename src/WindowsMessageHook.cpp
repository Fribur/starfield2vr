...
//
//    m_post_message_hook = std::make_unique<FunctionHook>((void **)&PostMessageA, (void *) &WindowsMessageHook::onPostMessageA);
//    if(!m_post_message_hook->create()) {
//        spdlog::error("Failed to hook PostMessageA");
//    }

    m_clip_cursor_hook = std::make_unique<FunctionHook>((void **)&ClipCursor, (void *) &WindowsMessageHook::onClipCursor);
    if(!m_clip_cursor_hook->create()) {
...
BOOL WindowsMessageHook::onPostMessageA(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
    auto original_fn = g_windows_message_hook->m_post_message_hook->get_original<decltype(WindowsMessageHook::onPostMessageA)>();
    return original_fn(hWnd, Msg, wParam, lParam);
}

BOOL WindowsMessageHook::onClipCursor(LPRECT lpRect)
{
    auto original_fn = g_windows_message_hook->m_clip_cursor_hook->get_original<decltype(WindowsMessageHook::onClipCursor)>();
    if(g_windows_message_hook->on_clip_cursor) {
        g_windows_message_hook->on_clip_cursor(&lpRect);
    }
    return original_fn(lpRect);
}
