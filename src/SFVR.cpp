...
std::unique_ptr<SFVR> g_framework{};

void SFVR::hook_monitor() {
...
//    auto& d3d11 = get_d3d11_hook();
    ...
    if (d3d12 == nullptr || (renderer_type == SFVR::RendererType::D3D12 && d3d12 != nullptr && !d3d12->is_inside_present()))
    {
...
SFVR::SFVR(HMODULE reframework_module)
    : m_reframework_module{reframework_module}
    , m_game_module{GetModuleHandle(0)}
//    , m_logger{spdlog::basic_logger_mt("SFVRFramework", (get_persistent_dir("sfvr_log.txt")).string(), true)}
    {
...
void SFVR::run_imgui_frame(bool from_present) {
...
void SFVR::on_frame_d3d12() {
    ...
    if (is_init_ok) {
        ...
        if (!std::exchange(m_created_default_cfg, true)) {
            if (!fs::exists({utility::widen(get_persistent_dir("sfvr_config.txt").string())})) {
                save_config();
            }
        }
    }
...
void SFVR::on_post_present_d3d12() {
...
void SFVR::patch_set_cursor_pos() {
...
void SFVR::remove_set_cursor_pos_patch() {
    std::scoped_lock _{ m_patch_mtx };
...
bool SFVR::on_clip_cursor(LPRECT* lpRect) {
    if(m_draw_ui) {
        *lpRect = nullptr;
        return true;
    }
...
bool SFVR::on_message(HWND wnd, UINT message, WPARAM w_param, LPARAM l_param) {
    m_last_message_time = std::chrono::steady_clock::now();
    ...
    case WM_KEYDOWN:
    case WM_SYSKEYDOWN: {
        const auto menu_key = SFVRConfig::get()->get_menu_key()->value();
...
// this is unfortunate.
void SFVR::on_direct_input_keys(const std::array<uint8_t, 256>& keys) {
...
std::filesystem::path SFVR::get_persistent_dir() {
...
void SFVR::save_config() {
    std::scoped_lock _{m_config_mtx};

    spdlog::info("Saving config sfvr_config.txt");
    ...
    try {
        if (!cfg.save((get_persistent_dir() / "sfvr_config.txt").string())) {
...
void SFVR::set_draw_ui(bool state, bool should_save) {
    std::scoped_lock _{m_config_mtx};
    ...
    if (m_game_data_initialized) {
        SFVRConfig::get()->get_menu_open()->value() = state;
    }
...
int SFVR::add_font(const std::filesystem::path& filepath, int size, const std::vector<ImWchar>& ranges) {
    ...
    m_additional_fonts.emplace_back(SFVR::AdditionalFont{filepath, size, ranges, (ImFont*)nullptr});
    m_fonts_need_updating = true;

    return m_additional_fonts.size() - 1;
}

void SFVR::update_fonts() {
...
void SFVR::invalidate_device_objects() {
...
void SFVR::draw_ui() {
    ...
    ImGui::GetIO().MouseDrawCursor = m_draw_ui || SFVRConfig::get()->is_always_show_cursor();
    ImGui::GetIO().ConfigFlags |= ImGuiConfigFlags_NoMouseCursorChange; // causes bugs with the cursor

    if (!m_draw_ui) {
        // remove SetCursorPos patch
        if (!SFVRConfig::get()->is_always_show_cursor()) {
    ...
    bool is_open = true;
    ImGui::Begin("SFVR Options", &is_open);
...
//    ImGui::Text("Author: mutar");
//    ImGui::Text("Inspired by the Kanan project.");
//    ImGui::Text("https://github.com/mutars/");
...
//        };
        ...
        static std::array<License, 0> licenses{
        };
...
void SFVR::set_imgui_style() noexcept {
...
//            }
        ...
        static const auto imgui_ini = (get_persistent_dir() / "sfvr_ui.ini").string();
...
bool SFVR::initialize_game_data() {
...
bool SFVR::initialize_windows_message_hook() {
    if (m_wnd == 0) {
        return false;
    }
...
bool SFVR::first_frame_initialize() {
    const bool is_init_ok = m_error.empty() && m_game_data_initialized;
...
#ifdef SFVR_EXPERIMENTAL
    m_smaa.setup(device, bb_desc);
#endif

#ifdef _DEBUG
//    TAADebugView::Get()->Init(device);
#endif
    return true;
}

void SFVR::deinit_d3d12() {
...
#ifdef SFVR_EXPERIMENTAL
    m_smaa.reset();
//    UpscalerAfrNvidiaModule::Get()->ResetD3D12();
//    TAADebugView::Get()->Reset();
#endif
    m_d3d12.cmd_ctxs.clear();
