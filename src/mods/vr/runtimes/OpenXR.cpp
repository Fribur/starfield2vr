...
//            SPDLOG_INFO("Eye texture proportion scale: {} by {}", eye_width_adjustment, eye_height_adjustment);
    ...
    for (auto i = 0; i < 2; ++i) {
        ...
        this->raw_projections[i][0] = tan(fov.angleLeft);
        this->raw_projections[i][1] = tan(fov.angleRight);
        this->raw_projections[i][2] = tan(fov.angleUp);
        this->raw_projections[i][3] = tan(fov.angleDown);

        frustums[i] = get_mat(i);
        // Update projection matrix
        XrMatrix4x4f_CreateProjection((XrMatrix4x4f*)&this->projections[i], GRAPHICS_D3D, frustums[i].x,frustums[i].y, frustums[i].z, frustums[i].w, nearz, farz);
...
std::optional<std::string> OpenXR::initialize_actions(const std::string& json_string) {
    ...
    for (const auto& controller : s_supported_controllers) {
        ...
        filename = (SFVR::get_persistent_dir() / filename).string();
