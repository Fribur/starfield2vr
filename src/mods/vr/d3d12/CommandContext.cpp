...
void CommandContext::copy_region(ID3D12Resource* src, ID3D12Resource* dst, D3D12_BOX* src_box, D3D12_RESOURCE_STATES src_state, D3D12_RESOURCE_STATES dst_state) {
    copy_region(src, dst, 0, 0, src_box, src_state, dst_state);
}

void CommandContext::copy_region(ID3D12Resource* src, ID3D12Resource* dst, UINT DstX, UINT DstY, D3D12_BOX* src_box, D3D12_RESOURCE_STATES src_state, D3D12_RESOURCE_STATES dst_state) {
    std::scoped_lock _{this->mtx};
    ...
    this->cmd_list->CopyTextureRegion(&dst_loc, DstX, DstY, 0, &src_loc, src_box);
