...
//    if(m_fxaa_enabled && !vr->m_left_trigger_down) {
//        m_fxaa.Apply(eye_texture.Get());
//        eye_texture = m_fxaa.m_render_target.texture.Get();
//    }
//
//    if(m_ssaa_enabled && !vr->m_left_trigger_down) {
//        m_ssaa.Apply(eye_texture.Get());
//        eye_texture = m_ssaa.m_render_target.texture.Get();
//    }

//    if(!vr->m_left_trigger_down) {
//        m_smaa.Apply(eye_texture.Get());
//        eye_texture = m_smaa.m_render_target.texture.Get();
//    }
    ...
    if (frame_count % 2 == vr->m_left_eye_interval) {
        ...
        if (runtime->is_openxr() && vr->m_openxr->ready()) {
            ...
            src_box.back = 1;
            if(true) {
                m_openxr.copy(0, eye_texture.Get(), D3D12_RESOURCE_STATE_RENDER_TARGET, &src_box);
            } else {
        ...
        if (runtime->is_openxr() && vr->m_openxr->ready()) {
            ...
            src_box.back = 1;
            if(true) {
                m_openxr.copy(1, eye_texture.Get(), D3D12_RESOURCE_STATE_RENDER_TARGET, &src_box);
            } else {
...
std::optional<std::string> D3D12Component::OpenXR::create_swapchains() {
    ...
    for (auto i = 0; i < openxr->views.size(); ++i) {
        ...
        spdlog::info("[VR] Backbuffer Format: {}", backbuffer_desc.Format);
