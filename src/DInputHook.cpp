...
bool DInputHook::hook() {
    ...
    auto get_device_data = (*(uintptr_t**)device)[10];

    spdlog::info("Got IDirectInputDevice::GetDeviceState {:x}", get_device_state);
    spdlog::info("Got IDirectInputDevice::GetDeviceData {:x}", get_device_data);
    ...
    m_get_device_data_hook = make_unique<FunctionHook>(get_device_data, (uintptr_t)&DInputHook::get_device_data
                                                        );
...
HRESULT WINAPI DInputHook::get_device_data(IDirectInputDevice* device, DWORD cbObjectData, LPDIDEVICEOBJECTDATA rgdod, LPDWORD pdwInOut, DWORD dwFlags) {
    return g_dinput_hook->get_device_data_internal(device, cbObjectData, rgdod, pdwInOut, dwFlags);
}

HRESULT DInputHook::get_device_data_internal(IDirectInputDeviceA* pA, DWORD i, LPDIDEVICEOBJECTDATA pDideviceobjectdata, LPDWORD pInt, DWORD i1)
{
    spdlog::debug("getDeviceData");

    auto original_get_device_data = (decltype(DInputHook::get_device_data)*)m_get_device_data_hook->get_original();

    return original_get_device_data(pA, i, pDideviceobjectdata, pInt, i1);
}
